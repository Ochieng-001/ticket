...
import { QrCode, Calendar, MapPin, CreditCard, Hash, Crown, Star, Sparkles, Download } from "lucide-react";
...
export function TicketCard({ ticket, eventName, eventDate, venue }: TicketCardProps) {
  ...
  useEffect(() => {
    const generateQRCode = async () => {
      ...
      try {
        ...
        console.log('TicketCard: Starting QR generation for:', { ticket, eventName, eventDate });
  ...
  const getTicketTypeLabel = (type: TicketType) => {
    console.log('TicketCard - getTicketTypeLabel called with type:', type, 'TicketType enum:', TicketType);
    switch (type) {
      case TicketType.REGULAR: return "Regular";
      case TicketType.VIP: return "VIP";
      case TicketType.VVIP: return "VVIP";
      default: 
        console.log('TicketCard - Defaulting to Regular for type:', type);
        return "Regular";
    }
  };
  ...
  const getTicketTypeIcon = (type: TicketType) => {
    switch (type) {
      case TicketType.REGULAR: return <Star className="w-4 h-4" />;
      case TicketType.VIP: return <Crown className="w-4 h-4" />;
      case TicketType.VVIP: return <Sparkles className="w-4 h-4" />;
  ...
  const getTicketTypeGradient = (type: TicketType) => {
    switch (type) {
      case TicketType.REGULAR: return "from-blue-500 to-indigo-600";
      case TicketType.VIP: return "from-purple-500 to-pink-600";
      case TicketType.VVIP: return "from-yellow-400 via-orange-500 to-red-500";
  ...
  const getStatusBadge = () => {
    if (ticket.isUsed) {
      return <Badge variant="destructive" className="rounded-full bg-red-500">Used</Badge>;
    }
    if (eventDate < new Date()) {
      return <Badge variant="secondary" className="rounded-full bg-gray-500">Expired</Badge>;
    }
    return <Badge className="bg-green-500 text-white rounded-full">Valid</Badge>;
  };

  return (
    <Card className="group overflow-hidden hover:shadow-2xl transition-all duration-300 border-0 shadow-lg bg-white">
      {/* Ticket Header with Gradient */}
      <div className={`bg-gradient-to-r ${getTicketTypeGradient(ticket.ticketType)} text-white p-6 relative overflow-hidden`}>
        {/* Background Pattern */}
          ...
          <div className="flex justify-between items-start mb-4">
            <div className="flex-1">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm font-semibold opacity-90">{getTicketTypeLabel(ticket.ticketType)} Access</span>
              </div>
            </div>
            
            <div className="text-right ml-4">
              <p className="text-xs opacity-75">Ticket ID</p>
              <p className="font-mono font-bold text-lg">#{ticket.ticketId.toString().padStart(6, '0')}</p>
            </div>
          </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 h-4 bg-white">
          <div className="flex justify-between items-center h-full px-4">
            {Array.from({ length: 15 }, (_, i) => (
              <div key={i} className={`w-2 h-2 rounded-full bg-gradient-to-r ${getTicketTypeGradient(ticket.ticketType)}`}></div>
            ))}
          </div>
        </div>
      </div>
      ...
      <CardContent className="p-6 bg-gradient-to-b from-gray-50 to-white">
        <div className="grid grid-cols-1 gap-4">
          ...
          <div className="flex items-center space-x-3 p-3 bg-white rounded-lg shadow-sm border border-gray-100">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center space-x-2">
                <p className="font-semibold text-gray-900">{getTicketTypeLabel(ticket.ticketType)}</p>
                <Badge 
                  className={`rounded-full text-xs ${
                    ticket.ticketType === TicketType.REGULAR ? 'bg-blue-100 text-blue-800' :
                    ticket.ticketType === TicketType.VIP ? 'bg-purple-100 text-purple-800' :
          ...
          <div className="flex items-center space-x-3 p-3 bg-white rounded-lg shadow-sm border border-gray-100">
            ...
            <div>
              ...
              <p className="font-bold text-xl text-gray-900">{formatPrice(ticket.purchasePrice)}</p>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-white to-gray-50 rounded-xl p-6 border-2 border-dashed border-gray-200">
            <div className="text-center">
              <div className="flex items-center justify-center mb-4">
                ...
                <h3 className="text-lg font-bold text-gray-900">Ticket QR Code</h3>
              </div>
              ...
              {isGeneratingQR ? (
                ...
                <div className="flex flex-col items-center space-y-4">
                  ...
                  <p className="text-xs text-gray-600 text-center max-w-sm">
                    Scan this QR code at the event entrance for quick verification
                  </p>
                  <Button
                    variant="outline"
                    size="sm"
                ...
                <div className="flex flex-col items-center space-y-3">
                  ...
                  {qrError && (
                    <p className="text-xs text-red-400 text-center max-w-sm">{qrError}</p>
                  )}
                </div>
              )}
            </div>
            ...
            onClick={() => {
              if (navigator.share && qrCodeDataURL) {
                navigator.share({
                  ...
                  files: [new File([qrCodeDataURL], `ticket-${ticket.ticketId}.png`, { type: 'image/png' })]
                });
              } else {
                // Fallback: copy ticket info to clipboard
                navigator.clipboard.writeText(`Ticket #${ticket.ticketId} for ${eventName} at ${venue} on ${eventDate.toLocaleDateString()}`);
              }
            }}
          >
            Share
        ...
        <div className="mt-4 p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
          <p className="text-xs text-blue-600 font-medium">🔗 Verified on Blockchain</p>
          <p className="text-xs text-blue-500 mt-1">This ticket is secured by smart contract technology</p>
        </div>
      </CardContent>
    </Card>
  );
